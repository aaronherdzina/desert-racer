...
func set_unique_details():
	details.title = 'CURVE DOWN RIGHT 2'
	details.move_distance = 2
	details.type = ['move', 'curve']
	details.move_dir = 'down_right'
	details.description = 'Move ' + str(details.move_dir).replace('_', ' and ') + ' ' + str(details.move_distance)
