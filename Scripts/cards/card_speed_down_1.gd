...
var remote_set = false # when setting new vals (rarely done) 
...
func set_unique_details():
	details.title = 'SPEED DOWN 1'
	details.speed_change = -1
	details.type = ['speed']
	details.description = 'Decrease speed ' + str(abs(details.speed_change))
