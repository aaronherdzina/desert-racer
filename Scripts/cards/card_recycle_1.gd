...
func set_unique_details():
	details.title = 'RECYCLE TERRAIN 5'
	details.buff_length = 5
	details.buff_value = 5
	details.buff_type = 'regen_obj'
	details.type = ['buff']
	details.description = 'Every destroyed piece of terrain restores +' + str(details.buff_value) + ' stability for ' + str(details.buff_length) + ' rounds'
